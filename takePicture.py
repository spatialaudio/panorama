...
from pythonPanorama.varisphear import varisphear


def arg_options():

    #init options
    parser = argparse.ArgumentParser(description='Calculate the motorpositions of the VariSphear depending on the aperature angles of the camera and take pictures. Final this pictures will be saved in the output directory spezifed in "panorama.cfg". ')
    
    parser.add_argument('--angle',type=int, nargs=2,dest='angle',default=None, metavar=('VERTICAL','HORIZONTAL'), 
                   help='Set the vertical aperature angle and the horizontal aperature angle for calculations. This values will be saved in "panorama.cfg" unless "-t" is set. ( default: values will be read from "panorama.cfg instead" )')
    ...
    parser.add_argument('-n','--noPicture',dest='noPicture',action='store_true',help='Skip the step of driving to each position and taking pictures')
...
def main():
    ...
    calc.showStatistic(angle_v,angle_h,dir_output)    
    # Mapping of angles to Motorpositions
    li_motorTop=variSphear.mapMotorTop(angle_v)
    li_motorBase=variSphear.mapMotorBase(angle_h)
    ...
    if(not args.noPicture):
        variSphear.takingPictures(li_motorTop,li_motorBase,dir_output,path_cfg)
