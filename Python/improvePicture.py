...
def arg_options():
    ...
    #init options
    parser = argparse.ArgumentParser(description='Load all pictures from the directory spezifed in "panorama.cfg" and improve them by removing radial disortions and correcting chromatic aberrations. For this purpose we use fulla ( a subprogram of hugin ). The quality of improvement depends on the quality of mesaured lens correction parameters a, b and c.')
    
    parser.add_argument('--directory', dest='directory',default=None,metavar=('DIRECTORY'),
                   help='Set the realtive path to directory. This value will be saved in "panorama.cfg" unless "-t" is set. ( default: path will be read from "panorama.cfg instead " ) ')
    ...
    parser.add_argument('--fulla', dest='fulla',default=None,metavar=('FULLA'),
                   help='Specifies the realitive path to fulla ( default: path will be read from "panorama.cfg instead " ) ')

    parser.add_argument('--abc',type=float, nargs=3,dest='abc',default=None, metavar=('A','B','C'), 
                   help='Set the lens correction parameters a, b and c for the improvement . This values will be saved in "panorama.cfg" unless "-t" is set. ( default: values will be read from "panorama.cfg instead" )')
    
    parser.add_argument('-t','--tmp',dest='tmp',action='store_true',help='Prevent any changes of "panorama.cfg" done by this program ')

    parser.add_argument('-n','--noImprovement',dest='noImprovement',action='store_true',help='Skip the step of improving pictures')
...
def main():
    ...
    if(not args.noImprovement):
        improve.improvePic(param_a,param_b,param_c)    
