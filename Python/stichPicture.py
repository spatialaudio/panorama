...
import configparser
import argparse
from python_core_components.sticher import PicSticher
...
def arg_options():
    ...
    parser = argparse.ArgumentParser(
        description='''Load all corrected pictures
            ( pictures with suffix "_corr.JPG" )
            from given directory and stich them together.
            For this purpose enblend is used ,
            which will be installed with hugins.
            Type %prog -i to watch the requirments''')
    ...
    parser.add_argument(
        ...
        default=None,
        metavar=('ENBLEND'),
        help='''Specifies the realitive path to enblend
            ( default: path will be read from "panorama.cfg instead " )''')
    ...
    parser.add_argument(
        ...
        default=None,
        metavar=('STAGES'),
        help='''Specifies the number of vertical stages.
    ...
    parser.add_argument(
        ...
        dest='orginal',
        action='store_true',
        help='Load all orginal picture instead of the corrected ones')
    ...
    parser.add_argument(
        ...
        dest='noStich',
        action='store_true',
        help='Skip the step of stiching pictures')
...
def main():
    ...
    if(args.stages):
        ...
        stages = int(calc.calc_num_stages(config['camera']['vertical_angle']))

    # create a dictonary, which contains a list of a all vertical pictures for
    # a given horizontal step
    dict_picture = sticher.split_pics(args.orginal, stages)

    # show some statistics
    sticher.show_statistic(dict_picture)

    # stiching the Pictures
    if(not args.noStich):
        sticher.stich_pictures(dict_picture)
