...
class picSticher:
    ...
    def __init__(self,dir_output,enblend):
        ...
        if(not os.path.isfile(self.enblend)):
            stderr.write('\n ERROR @PicSticher: Can´t find enblend @%s !'%self.enblend)
            stderr.write('\n Type " find /* | grep enblend " to figure your path out !\n exit \n')    		
            exit()
    
    def splitPics(self,orginal,stages):
        ...
        if(( stages < 1 ) or ( numberPictures % stages != 0 )):
            ...
            stderr.write('\n " Pic_per_horizontalRotation * vertical_Stages == Number of Pictures " is not fullfilled.')
            stderr.write('\n   Number of Pictures = %d , Stages = %d '%(numberPictures,stages))
            stderr.write('\n Exit \n ' )            
            exit() 


        # the next steps depends on our order of taken pictures ( --> see showInfo )
        list_select.sort()   
    ...
    def showStatistic(self,dict_picture):

        print("\n °----------------------------------------°");
        print("\n Statistic [ Stiching ]:");
        print("\n Picture will be loaded from:\n   %s"%(self.dir_outputABS));
        print("\n Number of Pictures for stiching :\t %d"%(len(dict_picture)));
    ...
    def showInfo(self):
        ...
        print(" There are x vertical stages ( depending on the vertical aperature angle ).");
        print(" We start with the top stage");
        print(" Each vertical stage consist of one horizontal Rotation ( == 360° ).");
        print(" Each horizontal Rotation consist of y steps ( depending on horizontal aperature angle ).");
        print(" In each step we take one picture.");
        print(" The order of your pictures ( sorted by date ) must be the follow :");
        ...
        print('\n Additional to this the equation: \n" Pic_per_horizontalRotation * vertical_Stages == Number of Pictures "\n have to be fullfilled')
        print(' For this purpose the Number of vertical_Stages will be load from the config-file ( unless you specify it via --stages ).')
        print('\n If the pictures in your folder fullfill this requirements you can use "stichPicture" for stichung your 360° sphaere panorama. ')
        print('\n Picture that are created with the varispear and takePicture meets this requirements automatically.')                  
        print("\n °----------------------------------------°")

    def stichPictures(self,dict_picture):
        ...
        for horizontal_step in dict_picture:
            ...
            for picture in list_v:
                ...
                if(vertical_panorama==None):
                    vertical_panorama=picture
                    #subprocess.call([self.enblend,'-g '+a+':'+b+':'+c+':'+d+' -o _tesst',filenameABS])
